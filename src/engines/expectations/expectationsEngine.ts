...
export interface SurpriseAnalysis {
  ...
  unit: string;
  surpriseType: 'ABOVE' | 'IN_LINE' | 'BELOW' | 'NO_FORECAST' | 'UNAVAILABLE';
  surpriseDelta: number | null;
  percentageSurprise: number | null;
  directionSummary: string;
  monetaryPolicyImplication: string;
  classification: 'FACT' | 'EXPECTATION' | 'INTERPRETATION' | 'ENGINE_ANALYSIS';
}

/**
 * Expectations Engine
 * Evaluates Previous, Forecast, Actual, and determines Surprise direction and policy implication.
 * Avoids hardcoding universal dogmatic truths; instead contextualizes based on indicator characteristics.
 */
export function analyzeObservationExpectations(
  ...
  const pctDelta = obs.forecast !== 0 
    ? Math.round(((obs.actual - obs.forecast) / Math.abs(obs.forecast)) * 1000) / 10 
    : 0;

  let surpriseType: 'ABOVE' | 'IN_LINE' | 'BELOW' = 'IN_LINE';
  // Small tolerance band to define "in line"
  ...
  if (surpriseType === 'ABOVE') {
    ...
    if (obs.category === 'INFLATION') {
      monetaryPolicyImplication = 'Upside inflation surprise pressures central bank to prolong restrictive stance or delay rate cuts.';
    ...
    if (obs.category === 'INFLATION') {
      monetaryPolicyImplication = 'Disinflation surprise reinforces room for central bank monetary easing.';
    } else if (obs.category === 'EMPLOYMENT') {
      monetaryPolicyImplication = highIsHawkish 
